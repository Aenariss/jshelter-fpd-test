...
/// \cond (Exclude this section from the doxygen documentation. If this section is not excluded, it is documented as a separate function.)
{
	...
	// Add listener when new page is accessed to launch createReport()
	browser.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
		if (changeInfo.status === "complete" && tab.url) {
			...
			current_page_name = tab.url
			console.log("A new page has been accessed:", tab.url);
			trackCallersCustom(tabId);
		}
	});
